...
foreach ($rawarr as $key => $value) {
    if ($value['messageid'] != $previous_id) { // такое сообщение еще не встречалось
        ...
        <form name="message<?= $key ?>" method="post">
            <b><?= $value['username'] ?></b><br>
            <i><?= $value['messagedate'] ?></i><br>

            <?php if (isset($_REQUEST["ChangeMessage"]) && ($_REQUEST['id'] == $value['messageid']))
            ...
            <?php } else { // сообщение не редактируем, а показываем
                echo $value['text']; ?>
                <br>
                <input type="submit" name="ChangeMessage" value="Изменить">
            <?php } // показываем текст или форму редактирования сообщения
            ...
            <?php if (isset($_REQUEST["AddComment"]) && ($_REQUEST['id'] == $value['messageid']))
    ...
    <form name="comment<?= $key.'_'.$value['commentid'] ?>" method="get">
